-- Enable postgis extension for spatial operations (optional)
CREATE EXTENSION IF NOT EXISTS postgis;

-- Enable earthdistance extension for ll_to_earth function (fallback)
...
CREATE TABLE reports (
    ...
    geom GEOMETRY(POINT, 4326), -- PostGIS geometry column for spatial operations
...
-- Function to update geometry from latitude/longitude
CREATE OR REPLACE FUNCTION update_report_geometry()
RETURNS TRIGGER AS $$
BEGIN
    -- Update PostGIS geometry if coordinates are provided
    IF NEW.latitude IS NOT NULL AND NEW.longitude IS NOT NULL THEN
        BEGIN
            NEW.geom = ST_SetSRID(ST_MakePoint(NEW.longitude, NEW.latitude), 4326);
        EXCEPTION WHEN OTHERS THEN
            -- If PostGIS is not available, set geom to NULL
            NEW.geom = NULL;
        END;
    ELSE
        NEW.geom = NULL;
...
-- Add trigger for geometry updates
...
-- Spatial indexes with fallback options
DO $$
BEGIN
    -- Try PostGIS spatial index first
    BEGIN
        CREATE INDEX idx_reports_geom ON reports USING GIST(geom);
        RAISE NOTICE 'Created PostGIS spatial index on geometry column';
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'PostGIS spatial index failed, trying earthdistance index';
        
        -- Try earthdistance spatial index
        BEGIN
            CREATE INDEX idx_reports_location ON reports USING GIST(ll_to_earth(latitude, longitude));
            RAISE NOTICE 'Created earthdistance spatial index';
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Earthdistance index failed, creating simple coordinate indexes';
            
            -- Fallback to simple coordinate indexes
            ...
            RAISE NOTICE 'Created simple coordinate indexes';
        END;
    END;
END
$$;
...
-- Function to find nearby reports (with fallback for different spatial capabilities)
...
BEGIN
    -- Try PostGIS distance calculation first
    BEGIN
        RETURN QUERY
        SELECT 
            r.id,
            ...
            r.category,
            r.status,
            ROUND((ST_Distance(r.geom, ST_SetSRID(ST_MakePoint(target_lng, target_lat), 4326)) / 1000)::DECIMAL, 2) as distance_km
        FROM reports r
        WHERE r.geom IS NOT NULL
        AND ST_DWithin(r.geom, ST_SetSRID(ST_MakePoint(target_lng, target_lat), 4326), radius_km * 1000)
        ORDER BY ST_Distance(r.geom, ST_SetSRID(ST_MakePoint(target_lng, target_lat), 4326))
        LIMIT limit_count;
        
        RETURN;
    EXCEPTION WHEN OTHERS THEN
        -- Fallback to earthdistance if PostGIS fails
        BEGIN
            RETURN QUERY
            SELECT 
                r.id,
                ...
                r.category,
                r.status,
                ROUND((earth_distance(ll_to_earth(r.latitude, r.longitude), ll_to_earth(target_lat, target_lng)) / 1000)::DECIMAL, 2) as distance_km
            FROM reports r
            WHERE r.latitude IS NOT NULL 
            AND r.longitude IS NOT NULL
            AND earth_distance(ll_to_earth(r.latitude, r.longitude), ll_to_earth(target_lat, target_lng)) <= radius_km * 1000
            ORDER BY earth_distance(ll_to_earth(r.latitude, r.longitude), ll_to_earth(target_lat, target_lng))
            LIMIT limit_count;
            
            RETURN;
        EXCEPTION WHEN OTHERS THEN
            -- Final fallback to simple coordinate comparison
            RETURN QUERY
            SELECT 
                r.id,
                r.title,
...
-- View for agency dashboard statistics (CORRECTED)
CREATE VIEW agency_statistics AS
SELECT 
    u.agency,
    COUNT(DISTINCT u.id) as staff_count,
    ARRAY_AGG(DISTINCT assigned_cat.category) FILTER (WHERE assigned_cat.category IS NOT NULL) as all_categories,
...
FROM users u
LEFT JOIN LATERAL unnest(u.assigned_categories) as assigned_cat(category) ON true
LEFT JOIN reports r ON r.category = assigned_cat.category
