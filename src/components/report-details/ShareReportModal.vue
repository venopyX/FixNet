        ...
        <div class="claycard bg-surface rounded-3xl max-w-lg w-full relative z-10 shadow-2xl">
          <div class="p-8">
            ...
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-2xl font-bold text-text-primary">Share Report</h2>
              <button
                @click="$emit('close')"
            ...
            >
              <div class="flex items-start gap-4">
                <div class="flex-1">
                  <h3 class="font-semibold text-text-primary mb-1">{{ report.title }}</h3>
                  <p class="text-text-secondary text-sm">{{ report.location }}</p>
                  <div class="flex items-center gap-2 mt-2">
                    <StatusBadge :status="report.status" />
                    <span class="text-xs text-text-muted">ID: #{{ report.id }}</span>
                  </div>
                </div>
                <div v-if="report.photo_url" class="w-16 h-16">
                  <img
            ...
            <!-- Share URL -->
            <div class="mb-6">
              <label class="block text-sm font-medium text-text-secondary mb-3">Report URL</label>
              <div
...
const reportUrl = computed(() => {
  ...
  return `${window.location.origin}/report/${props.report.id}`
})
...
const copyUrl = async () => {
  try {
    ...
    copied.value = true
    showShareSuccess('Report URL copied to clipboard!')
...
const shareVia = (platform: string) => {
  ...
  const text = `Check out this community report: "${props.report.title}" at ${props.report.location} on FixNet!`
  ...
  switch (platform) {
    ...
    case 'facebook':
      ...
      break
    case 'email':
      shareUrl = `mailto:?subject=${encodeURIComponent('Community Report Share')}&body=${encodeURIComponent(`${text}\n\n${url}`)}`
      break
  }
...
const nativeShare = async () => {
  if (!props.report || !canNativeShare.value) return

  try {
    await navigator.share({
      title: `${props.report.title} - FixNet Report`,
      text: `Check out this community report: "${props.report.title}" at ${props.report.location} on FixNet!`,
      url: reportUrl.value,
    })
    showShareSuccess('Report shared successfully!')
  } catch (error) {
