<template>
    <div class="claycard bg-surface rounded-3xl overflow-hidden">
      ...
      <div class="bg-background/50 p-6 border-b border-border/50">
        ...
        <p class="text-text-secondary text-sm">
          {{ reportsWithCoordinates.length }} of {{ reports.length }} reports have location coordinates
        </p>
      </div>
      ...
      <div class="relative">
        ...
        >
          ...
          <div class="absolute bottom-2 left-2 text-xs text-text-muted bg-surface/80 px-2 py-1 rounded">
            Lat: {{ bounds.minLat.toFixed(4) }}
          </div>
          <div class="absolute bottom-2 left-2 text-xs text-text-muted bg-surface/80 px-2 py-1 rounded">
            Lng: {{ bounds.minLng.toFixed(4) }}
          </div>
          <div class="absolute bottom-2 right-2 text-xs text-text-muted bg-surface/80 px-2 py-1 rounded">
      ...
      <Teleport to="body">
        ...
        >
          <div class="claycard bg-surface border border-border/50 p-4 rounded-xl shadow-lg max-w-sm">
            ...
            <p class="text-sm text-text-secondary mb-2 line-clamp-2">{{ tooltip.report!.description }}</p>
  ...
  const getMarkerPosition = (report: Report) => {
    ...
    const x = ((report.longitude! - bounds.value.minLng) / (bounds.value.maxLng - bounds.value.minLng)) * 100
    const y = ((bounds.value.maxLat - report.latitude!) / (bounds.value.maxLat - bounds.value.minLat)) * 100
